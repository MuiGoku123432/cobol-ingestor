---
description: Strangler Fig migration with 4 specialist agents
mode: agent
tools: ["cobol-graph"]
---
...
The strangler fig pattern allows gradual migration with zero downtime. A routing facade intercepts calls to COBOL programs and redirects them to modern implementations as they become available.
...
2. **Translator** migrates programs starting with leaf nodes (no downstream calls). Use `get_migration_sequence` for order
...
- Reviewer can send programs back to Translator with specific gap findings
